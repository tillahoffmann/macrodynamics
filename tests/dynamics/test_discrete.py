...
def get_state(request, fixtures, key, default):
    ...
    fixtures : list[str]
        the fixture names to key on. If a fixture name does not appear in this list,
        state will be shared across different values of the fixture.
    key : str
        name of the state
    default :
        value to use if no shared state is available
    """
    key = (request.function.__name__, key) + tuple(
        request.getfixturevalue(fixture) for fixture in fixtures
    )
...
def test_discrete_diffusion_operator(discrete_diffusion_operator, request):
    ...
    np.testing.assert_array_less(
        np.diag(matrix), eps, "diagonal entries must be non-positive"
    )
    np.testing.assert_array_less(
        -eps, np.triu(matrix, 1), "upper diagonal must be non-negative"
    )
    np.testing.assert_allclose(matrix, matrix.T, err_msg="operator is not symmetric")

    # Get a cached state if available
    z0 = get_state(
        request, ["num_dims", "kernel"], "z0", np.random.normal(0, 1, (1, num_nodes))
    )
    grad = discrete_diffusion_operator.evaluate_gradient(z0)
    # Check that the average gradient is close to zero (total is conserved for
    # diffusion).
...
def test_discrete_diffusion_integration(discrete_diffusion_operator):
    ...
    dt = 0.0001
    num_steps = 10000
    z0, z_actual, _ = _test_integration(
        discrete_diffusion_operator, time=dt * num_steps
    )
    ...
    np.testing.assert_allclose(
        np.sum(z_desired), np.sum(z0), 1e-5, err_msg="number of walkers not conserved"
    )
...
def test_discrete_diffusion_integration_shape(
    discrete_diffusion_operator, integration_method, time
):
...
def test_discrete_averaging_operator(discrete_averaging_operator, request):
    ...
    np.testing.assert_array_less(
        -eps, np.triu(matrix, 1), err_msg="upper diagonal must be non-negative"
    )
    np.testing.assert_array_less(
        -eps, np.tril(matrix, -1), err_msg="lower diagonal must be non-negative"
    )
...
def test_discrete_oscillation_operator(discrete_oscillation_operator, request):
    # Get a cached state if available.
    z0 = get_state(
        request, ["num_dims", "kernel"], "z0", np.random.normal(0, 1, (2, num_nodes))
    )
