...
def connectivity(kernel, lin_x, periodic, num_dims):
    _, kernel_coordinate_tensor, domain = md.coordinate_tensors(
        *[lin_x] * num_dims, periodic=periodic
    )
...
def test_density(density, lin_dx):
    ...
    np.testing.assert_allclose(
        np.sum(density) * lin_dx ** np.ndim(density),
        num_nodes,
        err_msg="density does not integrate to number of nodes",
    )


def test_continuous_diffusion_operator(
    continuous_diffusion_operator, connectivity, density, lin_dx
):
    ...
    np.testing.assert_allclose(
        continuous_diffusion_operator.kernel_weight_x,
        1,
        err_msg="kernel_weight_x must be equal to one",
    )
    np.testing.assert_allclose(
        continuous_diffusion_operator.kernel_weight_y,
        density[None, None],
        err_msg="kernel_weight_y must equal to the density",
    )
    np.testing.assert_allclose(
        continuous_diffusion_operator.kernel,
        connectivity[None, None],
        err_msg="kernel must equal to connectivity",
    )


def test_continuous_averaging_operator(
    continuous_averaging_operator, connectivity, density, lin_dx
):
    # Evaluate the properties of the operator
    np.testing.assert_allclose(
        continuous_averaging_operator.weight,
        -1,
        err_msg="weight must be equal to minus one",
    )
    np.testing.assert_allclose(
    ...
    np.testing.assert_allclose(
        continuous_averaging_operator.kernel_weight_y,
        density[None, None],
        err_msg="kernel_weight_y must equal to the density",
    )
    np.testing.assert_allclose(
        continuous_averaging_operator.kernel,
        connectivity[None, None],
        err_msg="kernel must equal to connectivity",
    )


def test_continuous_diffusion_gradient(
    continuous_diffusion_operator, density, initial_conditions
):
...
def test_continuous_diffusion_integration(
    ...
    _, z_numeric, _ = _test_integration(
        continuous_diffusion_operator, initial_conditions
    )
...
def test_continuous_diffusion_integration_with_control(
    continuous_diffusion_operator, density, initial_conditions
):
    control = np.pi * np.ones_like(density)[None]
    _, z_numeric, _ = _test_integration(
        continuous_diffusion_operator,
        initial_conditions,
        time=np.sqrt(2),
        control=control,
    )
...
def test_continuous_diffusion_integration_shape(
    continuous_diffusion_operator, integration_method, time
):
    _test_integration_shape(continuous_diffusion_operator, integration_method, time)


def test_continous_oscillation_operator(
    continuous_oscillation_operator, density, coordinate_tensor, num_dims, periodic
):
    center = np.random.uniform(0, 1, num_dims)
...
def test_continuous_oscillation_optimal_control(
    continuous_oscillation_operator,
    density,
    coordinate_tensor,
    num_dims,
    periodic,
    ...
    # Define the inputs
    t = np.sqrt(2)
    ic = md.evaluate_gaussian_kernel(coordinate_tensor, 0.25, 1, 0.05**2)
    ic = [ic, np.zeros_like(ic)]
    setpoint = md.evaluate_gaussian_kernel(coordinate_tensor, 0.75, 1, 0.05**2)
...
def test_continuous_oscillation_supramatrix(
    ...
    grad2 = continuous_oscillation_operator.supramatrix.dot(ic.ravel()).reshape(
        ic.shape
    )
