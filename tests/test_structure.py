...
@pytest.mark.parametrize('num_dims, kernel, periodic, condensed', it.product(
    [1, 3],
    [gd.evaluate_gaussian_kernel, gd.evaluate_laplace_kernel, gd.evaluate_tophat_kernel, gd.evaluate_uniform_kernel],
    [True, False],
    [True, False],
))
def test_sample_adjacency(num_dims, kernel, periodic, condensed):
    ...
    adjacency = gd.sample_adjacency(coordinates, kernel, condensed=condensed)
...
def test_evaluate_distance(num_dims, periodic):
    ...
    xx = gd.coordinate_tensor(*[x] * num_dims)
    # Evaluate the distance to the origin
    distance = gd.evaluate_distance(0, xx, domain=(periodic * np.ones(num_dims)) if periodic else None)
