...
def assert_standard_sentence(value, type, name, obj):
    ...
    try:
        ...
        LOGGER.warn(f"documentation '{value}' for {type} `{name}` {ex} for {obj.__name__} at "
...
def autodoc_process_docstring(app, what, name, obj, options, lines):
    ...
    if undocumented:
        ...
        LOGGER.warn(f"parameters {undocumented} should be documented for {obj.__name__} at "
                    f"{get_code_location(obj)}")
    ...
    if missing:
        ...
        LOGGER.warn(f"parameters {missing} are documented but not defined for {obj.__name__} at "
                    f"{get_code_location(obj)}")
    ...
    if what in ('function', 'method'):
        ...
        could_return = any(line.startswith('return ') or line.startswith('raise NotImplemented')
                           for line in source)
        returns = parsed.get('Returns')
        for parameter in parsed.get('Returns'):
            assert_standard_sentence(parameter.desc, 'return value', parameter.name, obj)

        if could_return and not returns:
            LOGGER.warn(f"return values should be documented for {obj.__name__} at "
                        f"{get_code_location(obj)}")
        if not could_return and returns:
            LOGGER.warn(f"return values are documented but not defined for {obj.__name__} at "
                        f"{get_code_location(obj)}")
